...
__device__ void warpReduceDouble(volatile double *sdata, unsigned int tid)
{
  sdata[tid] += sdata[tid + 32];
  sdata[tid] += sdata[tid + 16];
  sdata[tid] += sdata[tid + 8];
...
__device__ void blockReduceDouble(volatile double *sdata)
{
  unsigned int tid = threadIdx.x;
  __syncthreads();
    ...
    const float Q_thresh)
{
  ...
  GPU_1D_KERNEL_LOOP(k, num_points)
  {
    ...
    const bool valid =
        valid_match_ind & (q > Q_thresh) & (ci > C_thresh) & (cj > C_thresh) & valid_u & valid_v & valid_z; // Check for valid image and depth
...
__global__ void calib_proj_kernel_pieces(
    const torch::PackedTensorAccessor32<float, 2, torch::RestrictPtrTraits> Twc,
    const torch::PackedTensorAccessor32<float, 3, torch::RestrictPtrTraits> Xs,
    const torch::PackedTensorAccessor32<float, 3, torch::RestrictPtrTraits> Cs,
    const torch::PackedTensorAccessor32<float, 2, torch::RestrictPtrTraits> K,
    ...
    const torch::PackedTensorAccessor32<bool, 3, torch::RestrictPtrTraits> valid_match,
    const torch::PackedTensorAccessor32<float, 3, torch::RestrictPtrTraits> Q,
    torch::PackedTensorAccessor32<double, 4, torch::RestrictPtrTraits> Hs,
    torch::PackedTensorAccessor32<double, 3, torch::RestrictPtrTraits> gs,
    torch::PackedTensorAccessor32<double, 1, torch::RestrictPtrTraits> costs,
    const int height,
    const int width,
    const int pixel_border,
    const float z_eps,
    ...
    const float d_diff_thresh)
{
  ...
  // notice that we only compute hessian w.r.t. the relative transformation
  float Jx[7];

  // hessians
  const int h_dim = 7 * (7 + 1) / 2;
  double hij[h_dim];

  double vij[7];

  double local_cost = 0.0f;
  ...
  GPU_1D_KERNEL_LOOP(k, num_points)
  {
    ...
    // Get measurement pixel
    const int u_target = ind_Xi % (width * subpixel_factor);
    const int v_target = ind_Xi / (width * subpixel_factor);

    const int64_t ind_Xi_orig = v_target / subpixel_factor * width + u_target/subpixel_factor;

    Xi[2] = Xs[ix][ind_Xi_orig][2];
    ...
    // Error (difference in camera rays)
    err[0] = u - u_target/float(subpixel_factor);
    err[1] = v - v_target/float(subpixel_factor);
    err[2] = zj_log - zi_log; // Log-depth

    // Weights (Huber)
    const float q = Q[block_id][k][0];
    const float ci = Cs[ix][ind_Xi_orig][0];
    const float cj = Cs[jx][k][0];
    const bool valid =
        valid_match_ind & (q > Q_thresh) & (ci > C_thresh) & (cj > C_thresh) & valid_u & valid_v & valid_z; // Check for valid image and depth
    ...
    // sij = sj / si;
    float downweight_factor = 1.0;
    if(d_diff_thresh>0 && Xj[2] * (*sij) > Xi[2] * d_diff_thresh) downweight_factor = 0.01;

    // Add back in sigma
    const float w_const_pixel = sqrt_w_pixel * sqrt_w_pixel * downweight_factor;
    const float w_const_depth = sqrt_w_depth * sqrt_w_depth * downweight_factor;
    w[0] *= w_const_pixel;
    w[1] *= w_const_pixel;
    w[2] *= w_const_depth;

    local_cost += (w[0] * err[0])*(w[0] * err[0]);
    local_cost += (w[1] * err[1])*(w[1] * err[1]);
    // local_cost += (w[2] * err[2])*(w[2] * err[2]);
    
    // x coordinate
    Jx[0] = fx * zj_inv;
    Jx[1] = 0.0;
    Jx[2] = -fx * x_div_z * zj_inv;
    Jx[3] = -fx * x_div_z * y_div_z;
    Jx[4] = fx * (1 + x_div_z * x_div_z);
    Jx[5] = -fx * y_div_z;
    Jx[6] = 0.0;
    ...
    for (int n = 0; n < 7; n++)
    {
      vij[n] += w[0] * err[0] * Jx[n];
    }

    // y coordinate
    Jx[0] = 0.0;
    Jx[1] = fy * zj_inv;
    Jx[2] = -fy * y_div_z * zj_inv;
    Jx[3] = -fy * (1 + y_div_z * y_div_z);
    Jx[4] = fy * x_div_z * y_div_z;
    Jx[5] = fy * x_div_z;
    Jx[6] = 0.0;
  ...
  for (int n = 0; n < 7; n++)
  {
    sdata[threadIdx.x] = vij[n];
    blockReduceDouble(sdata);
    if (threadIdx.x == 0)
    {
      gs[0][block_id][n] = sdata[0];
    }
  ...
  sdata[threadIdx.x] = local_cost;
  blockReduceDouble(sdata);
  if (threadIdx.x == 0)
  {
    costs[block_id] = sdata[0];
  }
...
void AlignCoreCalib::init(torch::Tensor _Twc, torch::Tensor _Xs, torch::Tensor _Cs,
  torch::Tensor _K,
  ...
  const int _max_iter,
  const float _delta_thresh, const int _subpixel_factor, const float _d_diff_thresh)
{
  Twc          = _Twc         ;
  Xs           = _Xs          ;
  Cs           = _Cs          ;
  ...
  ii           = _ii          ;
  jj           = _jj          ;
  idx_ii2jj    = _idx_ii2jj   ;
  valid_match  = _valid_match ;
  Q            = _Q           ;
  height       = _height      ;
  width        = _width       ;
  pixel_border = _pixel_border;
  z_eps        = _z_eps       ;
  sigma_pixel  = _sigma_pixel ;
  sigma_depth  = _sigma_depth ;
  C_thresh     = _C_thresh    ;
  Q_thresh     = _Q_thresh    ;
  max_iter     = _max_iter    ;
  delta_thresh = _delta_thresh;
  subpixel_factor   = _subpixel_factor  ;
  d_diff_thresh = _d_diff_thresh;
}

void AlignCoreCalib::hessian(torch::Tensor H, torch::Tensor v)
{
  auto opts = Twc.options();
  const int num_edges = ii.size(0);
  const int num_poses = Xs.size(0);
  ...
  Eigen::MatrixXd Ad(A.A);
  Eigen::VectorXd vd(A.b);
...
void AlignCoreCalib::hessian_pieces(torch::Tensor H, torch::Tensor v, torch::Tensor cost)
{
  auto opts = Twc.options();
  auto opts2 = at::TensorOptions().dtype(at::kDouble).device(at::kCUDA, 0);
  const int num_edges = ii.size(0);
  const int num_poses = Xs.size(0);
  const int n = Xs.size(1);
  ...
  // initialize buffers
  torch::Tensor Hs = torch::zeros({1, num_edges, pose_dim, pose_dim}, opts2);
  torch::Tensor gs = torch::zeros({1, num_edges, pose_dim}, opts2);

  // For debugging outputs
  torch::Tensor costs = torch::zeros({num_edges},opts2);

  calib_proj_kernel_pieces<<<num_edges, THREADS>>>(
      Twc.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
      Xs.packed_accessor32<float, 3, torch::RestrictPtrTraits>(),
      Cs.packed_accessor32<float, 3, torch::RestrictPtrTraits>(),
      K.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
      ...
      valid_match.packed_accessor32<bool, 3, torch::RestrictPtrTraits>(),
      Q.packed_accessor32<float, 3, torch::RestrictPtrTraits>(),
      Hs.packed_accessor32<double, 4, torch::RestrictPtrTraits>(),
      gs.packed_accessor32<double, 3, torch::RestrictPtrTraits>(),
      costs.packed_accessor32<double, 1, torch::RestrictPtrTraits>(),
      height, width, pixel_border, z_eps, sigma_pixel, sigma_depth, C_thresh, Q_thresh,subpixel_factor,d_diff_thresh);
  ...
  // Eigen::VectorXd vd(A.b);
    ...
    auto sizes = Hs.sizes();  // 获取尺寸
    // std::cout << "Tensor dimensions: ";
    // for (auto size : sizes) {
    //     std::cout << size << " ";  // 打印每个维度的大小
    // }
    // std::cout << std::endl;
    ...
    // std::cout << "Tensor dimensions: ";
    // for (auto size : sizes) {
    //     std::cout << size << " ";  // 打印3每个维度的大小
    // }
    // std::cout << std::endl;

    H.copy_(Hs);  // 将 Hs 内容写入传进来的 H
    v.copy_(gs);
    cost.copy_(costs);

  // auto H_accessor = H.accessor<double, 4>();
  // auto v_accessor = v.accessor<double, 3>();
...
// Unused
torch::Tensor AlignCoreCalib::retract(torch::Tensor _dx)
{
  auto opts = Twc.options();
  const int num_edges = ii.size(0);
  const int num_poses = Xs.size(0);
